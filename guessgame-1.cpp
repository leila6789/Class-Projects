...
//Precondition: user says yes to play one round
//Postcondition: user will get to choose level 
//Description:  asks user for level, will set up a round of the game to be played.
{
...
//Plays one round of the guess game
{
	...
	for (int i=0; i< playerList[count].max_guesses; i++)
	{
		...
		if  (InterpretGuess()==true)
		{
			...
			cout << "\nYou won the round, " << playerList[count].name << "!!!!\n";
	...
	cout << "\nThe solution was:  " << playerList[count].random_number<< endl;
}
...
//prints the player results onto the screen
{
	...
	for (int i = 0; i < count; i++) 
	{
	cout << playerList[i].name << "\t" << playerList[i].level << "\t" << 
	playerList[i].wonOrLost<< "\n";
	}
